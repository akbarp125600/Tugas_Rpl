...
class Products_promo extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $dataz["notif"] = $this->productmodel_promo->getNotif($this->session->userdata('user_id')); //copy ini
        $this->load->view("user/product/list_promo", $dataz);
    }
    ...
    public function add()
    {
		...
		if($this->input->post("id_promo")){
			...
			if($qr->num_rows() <= 0){
                ...
                redirect(base_url().'user/products_promo');
			}
			else{
        ...
        $this->load->view("user/product/new_promo");
    }

    /*public function edit($idz = null)
    {
        $id = $idz;
        if (!isset($id)) redirect('user/Products_promo');
